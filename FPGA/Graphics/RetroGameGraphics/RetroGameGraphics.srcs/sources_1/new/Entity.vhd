...
ARCHITECTURE Behavioral OF Entitys IS
	...
	SIGNAL YVGA : unsigned(9 DOWNTO 0); -- VGA current pixel number todo: add ofset
		...
		-- ROM block entity
	SIGNAL entityAdress   : unsigned(ENTITY_AMOUNT * (ENTITY_ROM_ADRESS_BIT_SIZE+1) - 1 DOWNTO 0);  -- RGB value for tile -- (OTHERS => '1') is transparrent pixel
...
BEGIN
    ...
    EntityPixels0 : EntityPixels GENERIC MAP(
		...
		dataVector   => dataVector((3 * (ENTITY_X_BIT_SIZE + ENTITY_Y_BIT_SIZE + ENTITY_NUMMER_BIT_SIZE)) - 1 DOWNTO (2 * (ENTITY_X_BIT_SIZE + ENTITY_Y_BIT_SIZE + ENTITY_NUMMER_BIT_SIZE))),
		Xcount   => Xcount,
		Ycount   => Ycount,
		entityAdress   => entityAdress(3 * ENTITY_ROM_ADRESS_BIT_SIZE) - 1 DOWNTO (2 * (ENTITY_ROM_ADRESS_BIT_SIZE))
	);    
